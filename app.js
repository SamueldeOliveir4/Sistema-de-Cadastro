console.log("Rodando o Sistema de Gerenciamento de Jogos");
console.log("A aplicacao esta se conectando com o banco de dados ")
/*
    Comandos git 

    git clone = para clonar o arquivo
    git pull = Traz altercoes feitas 
    git commit . -m "mensagem " = realizar o commit 
    git push  origin = enviar alteracoes para o repositorio 
    git log --online = mostra todos os logs das modificacoes 
    git restore --source "hash" = retorna o codigo para a versao escolhida
    git log -p = ver alteracoes do commit
*/